...
ResampleResult TorchActorResampler::Resample(const ble::BridgeState& state) {
  ...
  const auto belief = torch_actor_->GetBelief(obs);

  // std::cout << "enter sample." << std::endl;
  std::array<int, ble::kNumCards> deal{};
  while (true) {
    deal = SampleFromBelief(belief, state, obs);
    if (deal[0] != -1) {
      break;
    }
  }
  ...
  for (const auto& item : bidding_history) {
    ...
    const auto policy = torch_actor_->GetPolicy(obs);
    const auto action_uid = game->GetMoveUid(item.move);
    // std::cout << "action uid: " << action_uid << std::endl;
    const auto action_prob = policy.at("pi")[action_uid - ble::kBiddingActionBase].item<float>();
...
rela::TensorDict TorchActorResampler::MakeTensorDictObs(const ble::BridgeState& state) const {
...
std::array<int, ble::kNumCards> TorchActorResampler::SampleFromBelief(const rela::TensorDict& belief,
                                                                      const ble::BridgeState& state,
                                                                      const rela::TensorDict& obs) const {
  const auto belief_probs = belief.at("belief");
  const torch::Tensor basic_indices = torch::arange(0, ble::kNumCardsPerHand) * ble::kNumPlayers;
  // std::cout <<"basic_indices:\n" << basic_indices << std::endl;
  const int observation_tensor_size = encoder_.Shape()[0];
  const auto player_cards_feature = obs.at("s").narrow(0, observation_tensor_size - ble::kNumCards, ble::kNumCards);
  ...
  torch::Tensor deal_cards = torch::ones(ble::kNumCards, {torch::kInt32}).fill_(-1);
  // std::cout <<"deal cards:\n" << deal_cards << std::endl;
  // std::cout << "current player: " << state.CurrentPlayer() <<std::endl;
  deal_cards = deal_cards.scatter_(0, basic_indices + state.CurrentPlayer(), player_cards);
  // std::cout << "deal cards:\n" << deal_cards << std::endl;
  ...
  for (const int sample_relative_player : {2, 1, 3}) {
    ...
    torch::Tensor relative_player_pred = belief_probs.slice(0, start_index, end_index).clone();
    relative_player_pred *= 1 - selected_cards;
    if (torch::count_nonzero(relative_player_pred).item<int>() < ble::kNumCardsPerHand) {
      return {-1}; // sentinel
    }
    // Sample 13 cards
    torch::Tensor sample_cards = torch::multinomial(relative_player_pred, ble::kNumCardsPerHand, false);
    selected_cards = selected_cards.scatter_(0, sample_cards, 1);
    deal_cards = deal_cards.scatter_(0,
                                     basic_indices + (state.CurrentPlayer() + sample_relative_player) %
                                     ble::kNumPlayers,
                                     sample_cards.to(torch::kInt32));
    // std::cout << "deal cards:\n" << deal_cards << std::endl;
  }
  std::array<int, ble::kNumCards> ret{};
  std::copy(deal_cards.data_ptr<int>(), deal_cards.data_ptr<int>() + ble::kNumCards, ret.begin());
  // for(int i=0; i< ble::kNumCards; ++i) {
