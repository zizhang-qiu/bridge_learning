...
class BridgeStateWithoutHiddenInfo {
 public:
  ...
  bool operator==(const BridgeStateWithoutHiddenInfo &other) const {
    return dummy_hand_ == other.dummy_hand_ &&
        UidHistory() == other.UidHistory() &&
  ...
  size_t operator()(const BridgeStateWithoutHiddenInfo &state) const {
    ...
    hash_value ^= std::hash<bool>()(state.is_dealer_vulnerable_) + 0x9e3779b9 + (hash_value << 6) + (hash_value >> 2);
    hash_value ^=
        std::hash<bool>()(state.is_non_dealer_vulnerable_) + 0x9e3779b9 + (hash_value << 6) + (hash_value >> 2);
    for (const int uid : state.UidHistory()) {
      hash_value ^= std::hash<int>()(uid) + 0x9e3779b9 + (hash_value << 6) + (hash_value >> 2);
    }
...
std::ostream &operator<<(std::ostream &stream, const BridgeStateWithoutHiddenInfo &state);
