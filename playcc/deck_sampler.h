...
class DeckSampler {
 ...
 public:
  ...
  ble::BridgeCard SampleNotSuit(ble::Suit suit, std::mt19937 &rng){
    std::array<std::vector<ble::BridgeCard>, ble::kNumSuits> cards_by_suits = deck_.CardsBySuits();
    ...
    auto random_index = dis(rng);
    ble::BridgeCard sampled_card = legal_cards[random_index];
  ...
  ble::BridgeCard Sample(std::mt19937 &rng){
    std::vector<ble::BridgeCard> legal_cards = deck_.Cards();
