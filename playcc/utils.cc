...
deal StateToDDSDeal(const ble::BridgeState& state) {
  ...
  const int num_tricks_played = static_cast<int>(play_history.size()) /
                                ble::kNumPlayers;
  const int num_card_played_current_trick =
      static_cast<int>(play_history.size()) - num_tricks_played *
      ble::kNumPlayers;
...
std::vector<ble::BridgeMove> GetLegalMovesWithoutEquivalentCards(
  ...
  const auto legal_cards = ExtractCardsBySuitsFromCardsVec(
      state.Hands()[state.IsDummyActing()
                      ? state.GetDummy()
  ...
  auto card_value = [cards_by_suits](const ble::BridgeCard& card) -> int {
    const auto it = std::find(cards_by_suits[card.CardSuit()].begin(),
                              cards_by_suits[card.CardSuit()].end(), card);
    return static_cast<int>(std::distance(
        cards_by_suits[card.CardSuit()].begin(), it));
  };
  ...
  for (const ble::Suit suit : suits) {
    ...
    std::sort(legal_card_values_by_suits[suit].begin(),
              legal_card_values_by_suits[suit].end());
  }
  ...
  std::array<std::vector<int>, ble::kNumCards>
      remained_legal_card_values_by_suits{};
  for (const ble::Suit suit : suits) {
    remained_legal_card_values_by_suits[suit] = KeepLargestConsecutive(
        legal_card_values_by_suits[suit]);
  }
  ...
  for (const ble::Suit suit : suits) {
    for (const int value : remained_legal_card_values_by_suits[suit]) {
      const auto it = std::find(card_values_by_suits[suit].begin(),
                                card_values_by_suits[suit].end(), value);
      const size_t index =
          std::distance(card_values_by_suits[suit].begin(), it);
