...
from .jps_model import BridgeA2CModel, A2CModel, CommModel, BridgeA2CModel2
...
class A2CAgent(torch.jit.ScriptModule):
        ...
        def initializer(**kwargs):
            ...
            if self.model_type == "resnet":
                self.online_net = BridgeA2CModel(
                    ...
                    use_old_feature=self.use_old_feature,
                )
            elif self.model_type == "e2e":
                self.online_net = BridgeA2CModel2(
                    self.num_action,
                    self.embed_dim,
                    self.hid_dim,
                    self.num_blocks,
                ...
                self.online_net = A2CModel(
                    self.input_dim, self.hid_dim, self.num_action, self.num_blocks
                )
    ...
    def loss(
        ...
        if "pi" in reply:
            ...
            entropy = -(pi * logpi).sum(1).mean(0)
            entropy_err = -self.entropy_ratio * entropy
            behavior_pi = batch["behavior_pi"].clamp(
                min=self.min_prob, max=1 - self.min_prob
            )
