...
class BridgeA2CModel2(torch.jit.ScriptModule):
    def __init__(
        self, num_action, embed_dim, hidden_dim, num_blocks, num_rnn_layers, device=None
    ):
        super().__init__()
        ...
        self.bid_embed = nn.Embedding(
            self.num_bids, self.embed_dim, padding_idx=self.num_bids - 1
        )
        self.card_embed = nn.Embedding(
            self.deck_size + 1, self.embed_dim, padding_idx=self.deck_size
        )
        self.swish = Swish()

        self.linear_b = nn.Linear(self.embed_dim + self.num_players, self.hidden_dim)
        self.linear_p = nn.Linear(
            self.embed_dim + self.embed_dim + self.num_players, self.hidden_dim
        )

        self.rnn_b = nn.LSTM(
            self.hidden_dim, self.hidden_dim, num_layers=self.num_rnn_layers
        )
        self.rnn_p = nn.LSTM(
            self.hidden_dim, self.hidden_dim, num_layers=self.num_rnn_layers
        )
        ...
        self.linear1 = nn.Linear(self.input_dim, self.hidden_dim)
        self.hidden1 = LinearList(self.hidden_dim, self.num_blocks, act="swish")

        self.linear2 = nn.Linear(self.input_dim, self.hidden_dim)
        self.hidden2 = LinearList(self.hidden_dim, self.num_blocks, act="swish")
    ...
    def _extract_bidding_feature(self, bid: torch.Tensor):
        ...
        index = (
            ...
            .unsqueeze(-1)
            .expand(1, batch_size, self.hidden_dim)
    ...
    @torch.jit.script_method
    def _extract_playing_feature(self, play: torch.Tensor, contract: torch.Tensor):
        ...
        play_seq = torch.cat([c, play_c, play_s], dim=-1).transpose(0, 1).contiguous()
        # [seq_len, batch_size, embed_dim]
    ...
    def forward(
        ...
        x = torch.cat(
            [vul, stage, bid, contract, doubled, declarer, s, trick, play, legal_move],
            dim=1,
        )
