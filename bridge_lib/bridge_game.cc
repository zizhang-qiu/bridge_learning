...
BridgeGame::BridgeGame(const GameParameters& params) {
  params_ = params;
  is_dealer_vulnerable_ = ParameterValue(params_, "is_dealer_vulnerable",
                                         kIsDealerVulnerable);
  is_non_dealer_vulnerable_ = ParameterValue(params_,
                                             "is_non_dealer_vulnerable",
                                             kIsNonDealerVulnerable);
...
GameParameters BridgeGame::Parameters() const {
...
std::string BridgeGame::ToString() const {
  GameParameters params = params_;
  params["name"] = "bridge";
...
bool BridgeGame::IsPartnershipVulnerable(int partnership) const {
  ...
  return partnership == Partnership(dealer_)
           ? is_dealer_vulnerable_
           : is_non_dealer_vulnerable_;
}
